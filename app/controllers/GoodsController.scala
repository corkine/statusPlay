...
class GoodsController @Inject()(cc: ControllerComponents, gr: GoodsRepository,
   ...
   * @param take 限制 - 用于分页
   */
  def goodsAll(lastDay:Option[Int],recentFirst:Option[Boolean],hideRemove:Option[Boolean],skip:Option[Long],take:Option[Long]): Action[AnyContent] = Action.async { r =>
    authAdmin(r) flatMap {
      case Right(v) => Future(v)
      case Left(_) =>
        gr.allGoods(lastDay, recentFirst.getOrElse(true), hideRemove.getOrElse(false), skip.getOrElse(0), take.getOrElse(1000)) map { res =>
          Ok(Json.toJson(res))
        }
    }
  }
